...
class VueSalleEdit {
    ...
    this._erreur = {
      edtNum: {
        ...
        msg: {
          ...
          inconnu:
            "Le numéro ne peut contenir que des lettres et des chiffres.",
  ...
  init(form: TSalleEditForm): void {
    ...
    this.form.btnAjouterEquipt.onclick = function (): void {
      vueSalleEdit.ajouterEquiptClick();
    };
    this.form.btnValiderEquipt.onclick = function (): void {
      vueSalleEdit.validerEquiptClick();
    };
    this.form.btnAnnulerEquipt.onclick = function (): void {
      vueSalleEdit.annulerEquiptClick();
    };
  }
  detailDepartement(valeur: string): void {
    ...
    if (chaine.length > 0) {
      ...
      if (dept.codeDept !== "") { // département trouvé
        detail.textContent = dept.nomDept + "\r\n" + "Responsable : " +
          dept.respDept;
      } else {
        err.statut = "inconnu";
    ...
    for (let id in this._grille) {
      ...
      if (!affi) {
        ...
        balisea.onclick = function (): void {
          vueSalleEdit.modifierEquiptClick(id);
        };
        ...
        balisea.onclick = function (): void {
          vueSalleEdit.supprimerEquiptClick(id);
        };
        tr.insertCell().appendChild(balisea);
      }
  ...
  validerClick(): void {
    ...
    correct = this.traiteErreur(this._erreur.edtNum, this.form.lblNumErreur) &&
      correct;
    correct =
      this.traiteErreur(this._erreur.edtEtage, this.form.lblEtageErreur) &&
      correct;
    correct =
      this.traiteErreur(this._erreur.edtCodeDept, this.form.lblDeptErreur) &&
      correct;
    correct =
      this.traiteErreur(this._erreur.equipt, this.form.lblEquiptErreur) &&
      correct;
    const lesSalles = new LesSalles();
  ...
  validerEquiptClick(): void {
    ...
    correct =
      this.traiteErreur(
        this._erreur.listeEquipt,
        this.form.lblSelectEquiptErreur,
      ) && correct;
    correct = this.traiteErreur(this._erreur.edtQte, this.form.lblQteErreur) &&
      correct;
    if (correct) {
      const lesTypEquipts = new LesTypEquipts();
      // ajout visuel de la ligne dans la grille tabulaire de la liste des équipements d'une salle
      const unTypEquipt: UnTypEquipt = lesTypEquipts.byIdEquipt(
        this._form.listeEquipt.value,
      );
      const unTypEquiptBySalle: UnTypEquiptBySalle = new UnTypEquiptBySalle(
        unTypEquipt,
